...
OrderManagementSystem::OrderManagementSystem(std::string username, std::string password): username_{username}, password_{password}{
...
void OrderManagementSystem::loadData(){
    ...
    if(time_left_ms > 0) this->last_action_ms += min_time_;
    else this->last_action_ms = current_time_ms;
    if(time_left_ms <= 0)         return;
    else    std::this_thread::sleep_for(std::chrono::milliseconds(time_left_ms));
    return;
}
...
OrderQueue& OrderManagementSystem::getOrderQueue(){
    return order_queue_;
}